...
const Home = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get(`${getServerUrl()}/api/user/allusers`)
            .then((res) => {
                setUsers(res.data.result);
                const currUser = getLoggedInUser();
                let idx = res.data.result.findIndex(
                    (x) => x.emailId === currUser.emailId
                );
                setRank(idx);
    ...
    const logoutFunc = () => {
        swal("Logout from App").then((value) => {
            logOutUser();
            navigate("/login");
        });
    };
    ...
    return (
        <div>
            ...
            <section
                class="slice pt-md-4 pb-5 pb-0 ml-10 mr-10 mb-6 tablesec"
                style={{
                    borderRadius: "1%",
                    marginTop: "5%",
                    position: "relative",
            ...
            >
                <table class="table table-cards ">
                    <thead>
                        <tr>
                            <th style={{ fontSize: "20px" }} scope="col">
                                Rank
                            </th>
                            <th style={{ fontSize: "20px" }} scope="col">
                                Username
